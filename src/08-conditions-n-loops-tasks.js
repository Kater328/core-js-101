...
function getFizzBuzz(num) {
  const three = num % 3 ? '' : 'Fizz';
  const five = num % 5 ? '' : 'Buzz';
  return three + five || num;
}
...
function getFactorial(n) {
  let result = 1;
  for (let i = 1; i < n + 1; i += 1) result *= i;
  return result;
}
...
function getSumBetweenNumbers(n1, n2) {
  let result = 0;
  for (let i = n1; i < n2 + 1; i += 1) result += i;
  return result;
}
...
function isInsideCircle(circle, point) {
  const hypotenuse = Math.hypot(circle.center.x - point.x, circle.center.y - point.y);
...
function findFirstSingleChar(str) {
  const arr = str.split('');
  const result = arr.find((elem) => arr.indexOf(elem) === arr.lastIndexOf(elem));
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const number2 = a > b ? a : b;
  return `${bracket1}${number1}, ${number2}${bracket2}`;
}
